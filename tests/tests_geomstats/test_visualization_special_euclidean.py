...
class TestVisualization(tests.conftest.TestCase):
    ...
    def teardown_method():
        plt.close()
"""
#add_points, draw_points, plot_geodesic

class TestVisualization(tests.conftest.TestCase):
    def setup_method(self):
        self.n_samples = 10
        self.SE2_GROUP = SpecialEuclidean(n=2, point_type="vector")
        self.SE3_GROUP = SpecialEuclidean(n=3, point_type="vector")

        plt.figure()

    def test_plot_points_se2(self):
        points = self.SE2_GROUP.random_point(self.n_samples)
        points_mat = self.SE2_GROUP.matrix_from_vector(points)
        visualization.plot(points_mat, space="SE2_GROUP")
    
    @staticmethod
    def test_plot_geodesic_se2():
